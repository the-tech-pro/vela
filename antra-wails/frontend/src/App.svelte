<script lang="ts">
  ...
  const libraryDetailSortOptions = [
    { value: 'position', label: 'Playlist order' },
    { value: 'title', label: 'Title' },
    { value: 'artist', label: 'Artist' },
    { value: 'album', label: 'Album' },
  ];
  const startupDestinationOptions = [
  ...
  const historyRetentionOptions = [25, 50, 100, 250, 500, 1000]
    .map(value => ({ value: String(value), label: `${value.toLocaleString()} jobs` }));
  const libraryModeOptions = [
    { value: 'smart_dedup', label: 'Reuse local audio' },
    { value: 'full_albums', label: 'Download every copy' },
  ];
  ...
  function setLibraryDetailSort(event: CustomEvent<string>) {
    const next = event.detail;
    if (['position', 'title', 'artist', 'album'].includes(next)) {
      libraryDetailSort = next as typeof libraryDetailSort;
    }
  }

  function setStartupDestination(event: CustomEvent<string>) {
    if (startupDestinationOptions.some(option => option.value === event.detail)) {
      uiPreferences.startup_destination = event.detail as StartupDestination;
      handleSettingsChange();
    }
  }

  function setHistoryRetention(event: CustomEvent<string>) {
    const next = Number(event.detail);
    if ([25, 50, 100, 250, 500, 1000].includes(next)) {
      uiPreferences.completed_history_retention = next;
      handleSettingsChange();
    }
  }

  function setLibraryMode(event: CustomEvent<string>) {
    if (libraryModeOptions.some(option => option.value === event.detail)) {
      config.library_mode = event.detail;
      handleSettingsChange();
    }
  }

  function setDiscoveryRegion(event: CustomEvent<string>) {
    discoveryRegion = event.detail;
    config.apple_storefront = discoveryRegion;
    void loadDiscoveryGenres();
    void loadDiscoveryData();
  }

  function setDiscoveryGenre(event: CustomEvent<string>) {
    discoveryGenre = event.detail;
    void loadDiscoveryData();
  }
  ...
  $: connectedSelectedIPod = ipodDevices.find(device =>
    device.device_id === selectedIPodId && device.path === selectedIPodPath
  ) || null;
  $: selectedIPodDevice = connectedSelectedIPod
    || (
      selectedIPodSnapshot?.device_id === selectedIPodId
      && selectedIPodSnapshot?.path === selectedIPodPath
      ? selectedIPodSnapshot
      : null
    );
  $: selectedIPodConnected = Boolean(connectedSelectedIPod);
  $: writableIPodDevices = ipodDevices.filter(device =>
    device.filesystem_accessible !== false && device.write_ready
  );
  $: writableIPodOptions = [
    { value: '', label: 'No iPod' },
    ...writableIPodDevices.map(device => ({
      value: `${device.device_id}|${device.path}`,
      label: `${device.name} · ${device.model_family}`,
    })),
  ];
    ...
    if (uiDemoMode) {
      if (!ipodDevices.length) {
        ipodDevices = [{
          ...
          browse_only: false, needs_preparation: false, write_ready: true,
          filesystem_read_only: false, write_block_code: '', write_block_reason: '',
        }];
        selectedIPodId = ipodDevices[0].device_id;
        selectedIPodPath = ipodDevices[0].path;
  ...
  function reconcileSelectedIPod(devices: IPodDevice[]) {
    ...
    selectedIPodPath = selected.path;
  ...
  function removeEjectedIPod(event: CustomEvent<{ device_id: string; path: string }>) {
    ...
    if (selectedIPodId === device_id && selectedIPodPath === path) {
      selectedIPodId = '';
      selectedIPodPath = '';
  ...
  async function startDownload() {
    ...
    if (otherUrls.length > 0) {
      ...
      const target = writableIPodDevices.find(
        device => `${device.device_id}|${device.path}` === pendingDownloadIPodDestinationId,
      );
  ...
  <div class="app-shell" use:dismissMenusOnPointerDown>
    <aside class="sidebar">
      <nav aria-label="Main navigation">
        ...
        {#each ipodDevices as device (`${device.device_id}|${device.path}`)}<button class:active={currentPage === 'devices' && selectedIPodId === device.device_id && selectedIPodPath === device.path} on:click={() => openIPodDevice(device)}><Smartphone size={18}/><span>{device.name || 'iPod'}</span></button>{/each}
      ...
      <main class="page-content">
        {#if currentPage === 'library'}
          {#if showAppleLibraryDetail && appleLibraryDetail}
            <section class="library-detail-page">
              ...
              <div class="detail-track-tools"><div class="search-bar detail-search"><Search size={16}/><input bind:value={libraryDetailFilter} placeholder="Search songs" /></div><SelectField id="library-detail-sort" ariaLabel="Sort songs" value={libraryDetailSort} options={libraryDetailSortOptions} on:change={setLibraryDetailSort} /><button class="secondary detail-order" on:click={() => libraryDetailDescending = !libraryDetailDescending}><ArrowUpDown size={16}/>{libraryDetailDescending ? 'Descending' : 'Ascending'}</button></div>
        ...
        {#if showSettings}
          <div class="settings-overlay" role="presentation" on:click|self={() => showSettings = false}>
            ...
            {#if settingsPage === 'general'}<section class="settings-section" id="settings-general"><div class="settings-heading"><div><p class="eyebrow">General</p><h2>Startup & activity</h2></div></div><div class="setting-row"><div><strong>Startup destination</strong><span>Choose the first page shown when Vela opens.</span></div><SelectField id="startup-destination" ariaLabel="Startup destination" value={uiPreferences.startup_destination} options={startupDestinationOptions} on:change={setStartupDestination} /></div><div class="setting-row"><div><strong>Remember last page</strong><span>Resume where you left off instead of the startup destination.</span></div><label class="switch"><input type="checkbox" bind:checked={uiPreferences.remember_last_page} /><span></span></label></div><div class="setting-row"><div><strong>Open Downloads when adding music</strong><span>Move to activity automatically after a request is queued.</span></div><label class="switch"><input type="checkbox" bind:checked={uiPreferences.open_downloads_on_add} /><span></span></label></div><div class="setting-row"><div><strong>Completion notifications</strong><span>Allow the desktop notification hook when a download completes.</span></div><label class="switch"><input type="checkbox" bind:checked={uiPreferences.completion_notifications} /><span></span></label></div><div class="setting-row"><div><strong>Device notifications</strong><span>Allow the desktop notification hook when an iPod connects or disconnects.</span></div><label class="switch"><input type="checkbox" bind:checked={uiPreferences.device_notifications} /><span></span></label></div><div class="setting-row"><div><strong>Completed history retention</strong><span>Keep a bounded number of completed jobs.</span></div><SelectField id="history-retention" ariaLabel="Completed history retention" value={String(uiPreferences.completed_history_retention)} options={historyRetentionOptions} on:change={setHistoryRetention} /></div><div class="setting-row"><div><strong>Music library folder</strong><span>{config.download_path || 'Not selected'}</span></div><button class="secondary" on:click={() => settingsPage = 'downloads'}>Location options</button></div><div class="setting-row"><div><strong>Library mode</strong><span>Reuse local audio while materializing every requested album and playlist folder.</span></div><SelectField id="library-mode" ariaLabel="Library mode" value={config.library_mode} options={libraryModeOptions} on:change={setLibraryMode} /></div></section>{/if}
            ...
            {#if settingsPage === 'discovery'}<section class="settings-section" id="settings-discovery"><div class="settings-heading"><div><p class="eyebrow">Discover</p><h2>Storefront & Genre</h2></div></div><div class="setting-row"><div><strong>Storefront</strong><span>Choose the Apple Music catalog region.</span></div><SelectField id="region" ariaLabel="Storefront" value={discoveryRegion} options={storefrontOptions} on:change={setDiscoveryRegion} /></div><div class="setting-row"><div><strong>Genre</strong><span>Filter discovery recommendations.</span></div><SelectField id="genre" ariaLabel="Genre" value={discoveryGenre} options={[{ value: '', label: 'All genres' }, ...discoveryGenres.map(genre => ({ value: String(genre.id), label: String(genre.name) }))]} on:change={setDiscoveryGenre} /></div></section>{/if}
...
<style>
  ...
  .launch-screen,.setup-screen,.app-shell { width:100%;height:100%;font-size:calc(14px * var(--ui-scale)); }
  ...
  input,textarea{border:1px solid var(--line);border-radius:10px;background:var(--surface-2);color:var(--text);outline:none;padding:10px 12px}input:focus,textarea:focus{border-color:var(--accent-border);box-shadow:0 0 0 3px var(--accent-soft)}
  ...
  .download-worker-summary{display:flex;align-items:center;justify-content:space-between;gap:16px;margin:16px 0 10px;padding:11px 13px;border:1px solid var(--line);border-radius:11px;background:var(--surface-2)}.download-worker-summary>div:first-child{display:grid;gap:2px}.download-worker-summary strong{font-size:12px}.download-worker-summary span,.worker-capacity{color:var(--muted);font-size:10px}.phase-counts{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:5px}.phase-counts span{padding:4px 7px;border-radius:99px;background:var(--surface);color:var(--muted)}:global(.download-track-list article).failed{background:var(--error-soft)}.measured-progress{display:grid;grid-template-columns:minmax(120px,1fr) auto;align-items:center;gap:9px}.measured-progress small{color:var(--muted);font-size:9px;white-space:nowrap}.failure-panel{display:grid;gap:7px;margin-top:14px;padding:12px;border:1px solid var(--error-border);border-radius:11px;background:var(--error-soft)}.failure-panel header{display:flex;align-items:center;justify-content:space-between}.failure-panel header>button{display:flex;align-items:center;gap:6px;padding:5px 8px;border-radius:7px;background:var(--surface-2);color:var(--muted)}.failure-panel>article{display:grid;grid-template-columns:18px 1fr;align-items:start;gap:8px;padding-top:7px;border-top:1px solid var(--line);color:var(--error-color,#ff453a)}.failure-panel>article>div{display:grid;gap:2px}.failure-panel>article span{color:var(--muted);font-size:10px;line-height:1.4}.worker-capacity{font-weight:500}
  ...
  .detail-primary-actions{display:flex;align-items:center;gap:8px;margin-top:14px}.detail-primary-actions .detail-download{margin-top:0}.detail-primary-actions .tool-menu{position:relative}.detail-download{height:38px;display:inline-flex;align-items:center;justify-content:center;gap:7px;line-height:1}.detail-download span{display:block;line-height:1}.detail-popover{position:absolute;top:44px;left:0}.detail-track-tools :global(.select-field),.detail-order{font-size:12px;line-height:1}
  :global(.library-detail-tracks article){position:relative;background:var(--track-surface);backdrop-filter:none;content-visibility:auto;contain:layout paint style}.track-more{position:relative}.track-more>button{width:30px;height:30px;display:grid;place-items:center;border-radius:50%;background:transparent}.track-popover{position:absolute;right:0;top:34px}.local-placeholder-art{display:grid;place-items:center;background:var(--surface-2);color:var(--faint)}
  .downloaded-release-grid{grid-template-columns:repeat(auto-fill,minmax(155px,1fr));gap:24px 18px}.local-detail-tracks{display:grid;gap:5px}:global(.local-detail-tracks>button){width:100%;min-height:58px;display:grid;grid-template-columns:28px 44px minmax(170px,1.35fr) minmax(130px,.7fr) minmax(160px,1fr) 52px;align-items:center;gap:10px;padding:6px 9px;border-radius:10px;background:var(--track-surface);color:var(--text);text-align:left;content-visibility:auto;contain:layout paint style}:global(.local-detail-tracks>button:hover),:global(.local-detail-tracks>button.playing){background:var(--track-surface-hover)}:global(.local-detail-tracks>button.playing){color:var(--accent)}:global(.local-detail-tracks img){width:44px;height:44px;border-radius:7px;object-fit:cover}:global(.local-detail-tracks>button>span),:global(.local-detail-tracks small){overflow:hidden;white-space:nowrap;text-overflow:ellipsis;color:var(--muted);font-size:11px}
  ...
  :global(.download-track-list) article.failed{background:var(--error-soft)}
  ...
  :global(.library-detail-tracks) article{position:relative;width:100%;height:58px;display:grid;grid-template-columns:28px 44px minmax(170px,1.35fr) minmax(130px,.7fr) minmax(160px,1fr) 42px 32px;align-items:center;gap:10px;padding:6px 9px;border-radius:10px;background:var(--track-surface);contain:layout paint style}
  :global(.library-detail-tracks) article:hover{background:var(--track-surface-hover)}
  ...
  :global(.local-detail-tracks) button{width:100%;height:58px;display:grid;grid-template-columns:28px 44px minmax(170px,1.35fr) minmax(130px,.7fr) minmax(160px,1fr) 52px;align-items:center;gap:10px;padding:6px 9px;border-radius:10px;background:var(--track-surface);color:var(--text);text-align:left}
  :global(.local-detail-tracks) button:hover,:global(.local-detail-tracks) button.playing{background:var(--track-surface-hover)}
